...
import { IconStar, IconTrash, IconMail, IconMailOpen, IconAttachment, IconAlertTriangle } from '../ui/Icons';
...
// Inline preview panel — shown when this row is the active preview
function PreviewPanel({ email, onViewDetail }) {
  ...
  return (
    <div className="email-preview-panel fade-up">
      {/* Spam notice — HCI: N9 Help & Documentation */}
      {email.isSpamDetected && (
        <div className="preview-notice-bar" style={{ background: '#fff3f3', borderBottom: '1px solid #fecdd3', color: '#b91c1c' }}>
          <IconAlertTriangle className="w-3.5 h-3.5" style={{ flexShrink: 0 }} />
          <span>This message was flagged as potential spam. Be cautious before clicking any links.</span>
        </div>
      )}

      {/* Body preview text */}
      <div style={{ padding: '12px 16px 8px', fontSize: '.875rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
        <div dangerouslySetInnerHTML={{ __html: email.body ?? email.preview }} />
      </div>

      {/* Image thumbnails — HCI: P4 Dual-Coding */}
      {images.length > 0 && (
        <div className="email-preview-images">
          {images.map((img, i) => (
            <div
              key={i}
      ...
      {/* Smart replies — HCI: N7 Flexibility & Efficiency */}
      {smartReplies.length > 0 && (
        <div className="smart-replies">
          {smartReplies.map(r => (
            <button key={r} className="smart-reply-btn" onClick={() => handleSmartReply(r)}>
              {r}
            </button>
          ))}
        </div>
      ...
      {/* View full email CTA — HCI: D1 Visibility */}
      <div className="preview-notice-bar" style={{ justifyContent: 'flex-end', background: 'var(--gray-50)' }}>
        <span style={{ fontSize: '.75rem', color: 'var(--text-muted)' }}>Preview mode</span>
        <button className="view-mail-btn" onClick={() => onViewDetail?.(email.id)}>
          View Mail &rsaquo;
        </button>
      </div>
    </div>
...
export function EmailRow({ email, isSelected, isChecked, isPreview, onSelect, onCheck, onViewDetail }) {
  const { toggleStar, archive, trash, markRead } = useEmailStore();
  const { showToast } = useUiStore();
  const [hovered, setHovered] = useState(false);

  const makeUndoToast = (msg, action) => showToast({ message: msg, type: 'info', undoAction: action });

  const handleTrash      = (e) => { e.stopPropagation(); trash([email.id]); makeUndoToast('Moved to Trash — recoverable for 30 days', 'trash'); };
  const handleToggleRead = (e) => { e.stopPropagation(); markRead([email.id], !email.isRead); };
  const handleStar       = (e) => { e.stopPropagation(); toggleStar(email.id); };
  ...
  return (
    <div role="listitem">
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
            {/* Spam warning icon — HCI: N9, P4 Dual-Coding */}
      ...
      {/* Inline preview panel — HCI: W4 Feature Exposure, N6 Recognition over Recall */}
